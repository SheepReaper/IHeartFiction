...
/// <summary>
/// Integration tests for GetAuthor using in-memory database
/// These tests verify the complete flow from database to response
/// </summary>
// [Collection(nameof(AuthorsTestCollection))]
public class GetAuthorIntegrationTests : BaseIntegrationTest, IConfigureServices<GetAuthorIntegrationTests>, IAsyncLifetime
{
    private readonly FictionDbContext _dbContext;
    private readonly GetAuthor _useCase;

    public GetAuthorIntegrationTests(IntegrationTestWebAppFactory factory) : base(factory)
    {
        _dbContext = _scope.ServiceProvider.GetKeyedService<FictionDbContext>(nameof(GetAuthorIntegrationTests)) ?? throw new Exception("DbContext not found");
    ...
    public async Task HandleAsync_WhenAuthorExists_ReturnsSuccessWithCorrectResponse()
    {
        ...
        response.UpdatedAt.Should().BeAfter(updatedAt.AddSeconds(-1));
    ...
    public async Task HandleAsync_WhenAuthorHasEmptyBio_ReturnsEmptyBio()
    {
        ...
        result.IsSuccess.Should().BeTrue();
        var response = result.Value!;
        response.Profile.Bio.Should().Be("");
    }
