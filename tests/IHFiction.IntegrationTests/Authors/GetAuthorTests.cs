...
/// <summary>
/// Integration tests for GetAuthor functionality
/// Tests the complete use case handler and response mapping with database
/// </summary>
// [Collection(nameof(AuthorsTestCollection))]
public class GetAuthorTests : BaseIntegrationTest, IConfigureServices<GetAuthorTests>, IAsyncLifetime
{
    private readonly FictionDbContext _dbContext;
    private readonly GetAuthor _useCase;

    public GetAuthorTests(IntegrationTestWebAppFactory factory) : base(factory)
    {
        _dbContext = _scope.ServiceProvider.GetKeyedService<FictionDbContext>(nameof(GetAuthorTests)) ?? throw new Exception("DbContext not found");
