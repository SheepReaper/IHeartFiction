...
/// <summary>
/// Database-specific integration tests for GetAuthorById
/// Tests PostgreSQL-specific behaviors that weren't possible with in-memory database
/// </summary>
// [Collection(nameof(AuthorsTestCollection))]
public class GetAuthorByIdDatabaseSpecificTests : BaseIntegrationTest, IConfigureServices<GetAuthorByIdDatabaseSpecificTests>, IAsyncLifetime
{
    private readonly GetAuthorById _useCase;
    private readonly FictionDbContext _dbContext;

    public GetAuthorByIdDatabaseSpecificTests(IntegrationTestWebAppFactory factory) : base(factory)
    {
        _dbContext = _scope.ServiceProvider.GetKeyedService<FictionDbContext>(nameof(GetAuthorByIdDatabaseSpecificTests)) ?? throw new Exception("DbContext not found");
    ...
    public async Task HandleAsync_WithPostgreSQLTimestampPrecision_HandlesCorrectly()
    {
        ...
        ((dynamic)result!).UpdatedAt.Should().BeCloseTo(DateTime.UtcNow, TimeSpan.FromSeconds(5));
    ...
    public async Task HandleAsync_WithPostgreSQLCaseHandling_RespectsCollation()
    {
        ...
        ((dynamic)result1!).Name.Should().Be("Test Author");
        ((dynamic)result2!).Name.Should().Be("test author");
    ...
    public async Task HandleAsync_WithLargeDataSet_PerformsEfficientlyWithPostgreSQL()
    {
        ...
        // Assert
        result.Should().NotBeNull();
        ((dynamic)result!).Name.Should().Be("Author 0500");
    ...
    public async Task HandleAsync_WithConcurrentAccess_HandlesPostgreSQLLocking()
    {
        ...
        var tasks = Enumerable.Range(0, 10)
            .Select(_ => Task.Run(async () =>
            {
                ...
                var dbContext = scope.ServiceProvider.GetKeyedService<FictionDbContext>(nameof(GetAuthorByIdDatabaseSpecificTests)) ?? throw new Exception("DbContext not found");
                var useCase = new GetAuthorById(dbContext);
        ...
        {
            result.Should().NotBeNull();
            ((dynamic)result!).Name.Should().Be("Concurrent Test Author");
        });
    }
    ...
    public async Task HandleAsync_WithPostgreSQLTextSearch_WorksCorrectly()
    {
        ...
        // Assert
        result.Should().NotBeNull();
        ((dynamic)result!).Profile.Bio.Should().Contain("café");
