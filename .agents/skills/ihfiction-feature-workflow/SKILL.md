...
- For persisted data resources, treat credentials as mutable cluster state. If auth fails, verify mounted volume identity and logs first, then probe live auth; do not assume env/secret values automatically match stored database hashes.
...
Run focused tests after each increment and broader checks after integration. Use the agent bootstrap in cloud/automated runners; in an already-configured local developer shell, prefer ordinary restoring builds and do not run the cloud bootstrap merely to use `--no-restore`. For API contract changes, perform a normal host-runtime API build so `openapi.json` is refreshed, then build the consumer that invokes client generation.

Bootstrap reuses any `.artifacts/packages/IHFiction.SourceGenerators*.nupkg` by default and must not clear the shared global NuGet package cache. When diagnosing resolution independently of an active IDE or build server, use a task-specific temporary `RestorePackagesPath`, then shut down .NET build servers before deleting that verified temporary directory. After source-generator changes, force publication with `agent-bootstrap.ps1 -ForceSourceGeneratorPackage` or `agent-bootstrap.sh --force-source-generator-package`; Bash publication requires `python3` for normalized NuGet package comparison. On Windows, use PowerShell for the actual preflight when Git Bash cannot see `dotnet`, and use Ubuntu WSL to validate Bash behavior. If WSL lacks a Linux SDK, translate every WSL path passed to Windows `dotnet.exe` with `wslpath -w`; a raw `/mnt/...` project path is parsed by MSBuild as an invalid switch.
...
When runtime behavior conflicts with source expectations, verify runtime freshness before code changes: rebuild relevant projects, restart/rebuild resources, and re-check live API payloads/filters prior to browser assertions.
